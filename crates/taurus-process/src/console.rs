...
//! here — [`crate::Tree`] included, which applies the flag its own way. A
//! spawn site that forgets is invisible on the platforms most of this is
//! developed on, and shows up only as a flicker on somebody else's machine.
//!
...
//! `taurus_tools::builtin::pty` and the known-gaps entry it points at.

/// `CREATE_NO_WINDOW` from the Win32 process creation flags.
///
/// Spelled out rather than taken from the `windows` crate so it can be checked
/// on every platform: the tests below assert it anywhere. Only Windows ever
/// applies it.
